...
void joint_callback(const void *msgin) {
  ...
  if (msg->points.size > 0 && msg->points.data[0].positions.size >= 4) {
    ...
    portENTER_CRITICAL(&mux);
    target1 = msg->points.data[0].positions.data[0] * (PULSES_PER_REV / 360.0) * GEAR_RATIO;
    target2 = msg->points.data[0].positions.data[1] * (PULSES_PER_REV / 360.0) * GEAR_RATIO;
    target3 = msg->points.data[0].positions.data[2] * (PULSES_PER_REV / 360.0) * GEAR_RATIO;
...
void init_traj_msg() {
  ...
  // joint_names
  traj_msg.joint_names.size = NUM_JOINTS;
  traj_msg.joint_names.capacity = NUM_JOINTS;
  traj_msg.joint_names.data = (rosidl_runtime_c__String*)malloc(NUM_JOINTS * sizeof(rosidl_runtime_c__String));
  ...
  // points[]
  traj_msg.points.size = MAX_POINTS;
  traj_msg.points.capacity = MAX_POINTS;
  traj_msg.points.data = (trajectory_msgs__msg__JointTrajectoryPoint*)malloc(MAX_POINTS * sizeof(trajectory_msgs__msg__JointTrajectoryPoint));

  for (int i = 0; i < MAX_POINTS; i++) {
    trajectory_msgs__msg__JointTrajectoryPoint__init(&traj_msg.points.data[i]);

    traj_msg.points.data[i].positions.data = (double*)malloc(NUM_JOINTS * sizeof(double));
    traj_msg.points.data[i].positions.size = NUM_JOINTS;
    traj_msg.points.data[i].positions.capacity = NUM_JOINTS;

    // Si necesitas reservar también velocities, hazlo aquí:
    /*
    traj_msg.points.data[i].velocities.data = (double*)malloc(NUM_JOINTS * sizeof(double));
    traj_msg.points.data[i].velocities.size = NUM_JOINTS;
    traj_msg.points.data[i].velocities.capacity = NUM_JOINTS;
    */
  }

  Serial.println("✅ traj_msg reservado para 128 puntos.");
}

void destroy_traj_msg() {
  Serial.println("[destroy_traj_msg] Liberando memoria de traj_msg...");

  // Liberar joint_names
  for (size_t i = 0; i < traj_msg.joint_names.size; i++) {
    if (traj_msg.joint_names.data[i].data != NULL) {
      free(traj_msg.joint_names.data[i].data);
    }
  }
  if (traj_msg.joint_names.data != NULL) {
    free(traj_msg.joint_names.data);
  }

  // Liberar header.frame_id
  if (traj_msg.header.frame_id.data != NULL) {
    free(traj_msg.header.frame_id.data);
  }

  // Liberar points
  for (size_t i = 0; i < traj_msg.points.size; i++) {
    trajectory_msgs__msg__JointTrajectoryPoint *point = &traj_msg.points.data[i];

    if (point->positions.data != NULL) {
      free(point->positions.data);
    }
    if (point->velocities.data != NULL) {
      free(point->velocities.data);
    }
    if (point->accelerations.data != NULL) {
      free(point->accelerations.data);
    }
    if (point->effort.data != NULL) {
      free(point->effort.data);
    }
  }

  if (traj_msg.points.data != NULL) {
    free(traj_msg.points.data);
  }

  Serial.println("✅ traj_msg completamente liberado.");
}
