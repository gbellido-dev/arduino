...
void setup() {
  ...
  digitalWrite(EN_PIN, LOW); // Activa driver
...
void loop() {
  if (Serial.available()) {
    ...
    switch (option) {
      case '1': moveRelative(3200*8); break;      // Una vuelta adelante
      case '2': moveRelative(-3200*8); break;     // Una vuelta atrás
      case '3': homeMotor(); break;             // Home
      case '4': stopMotor(); break;             // Stop motor
...
void sendCommandAndAutoDisplay(byte cmd) {
  ...
  if (expectedBytes == 2) {
    ...
    if (cmd == CMD_POSITION_ERROR) {
      // Mostrar en grados especiales para Position Error
      float degrees = value * 0.00559;
    ...
    if (cmd == CMD_MOTOR_POSITION || cmd == CMD_CUMULATIVE_PULSES) {
      // Motor Position o Cumulative Pulses → conversión normal a grados
      float degrees = value * (360.0 / 3200.0);
      Serial.print(" pasos (");
      Serial.print(degrees, 2);
      Serial.println("º)");
    } else {
...
void showMenu() {
  ...
  Serial.println("6 - Leer Cumulative Pulses (pulsos y grados)");
  //Serial.println("7 - Leer Motor Position (pulsos y grados)");
  Serial.println("8 - Leer Position Error (unidades y grados)");
